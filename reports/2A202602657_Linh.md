...
|---|---|---|---|
| **Task 1: Thu thập tài liệu pháp luật** | Khảo sát và tải 5 tài liệu pháp lý định dạng PDF (>1KB) gồm: Luật Du lịch 09/2017/QH14, Nghị định 168/2017/NĐ-CP, Quyết định 509/QĐ-TTg, Nghị định 45/2019/NĐ-CP, Quyết định 147/QĐ-TTg. | `src/task1_collect_legal_docs.py`, `data/landing/legal/` (commit `e9e9c9f`) | Done |
...
| **Đánh giá Benchmark & Báo cáo** | Thực nghiệm đo kiểm 4 metric Ragas (Faithfulness, Relevance, Recall, Precision), phân tích so sánh A/B giữa Dense-only và Hybrid + RRF, lập bảng phân tích ca lỗi. | `group_project/evaluation/RESULT.md`, `reports/RESULT.md` (commit `e9e9c9f`) | Done |
...
2. **Quyết định:** Loại bỏ triệt để các thành phần boilerplate (menu điều hướng, banner quảng cáo, mã nhúng HTML) khi chuẩn hóa Markdown từ các bài viết web.  
   ...
   **Trade-off:** Cần viết thêm logic làm sạch và regex tùy biến cho từng mẫu bài viết, nhưng nâng Context Precision trong đánh giá Ragas lên 0.874 (+6.9%).
...
- **Test đã dùng:** Chạy test chấp nhận dữ liệu tự động:
  ...
  ```
- **Kết quả:** `5 passed in 0.03s` (đạt 100%):
  - `test_corpus_has_required_legal_documents`: 5 file PDF hợp lệ (>1KB).
  - `test_corpus_has_required_news_with_metadata`: 7 file JSON có đầy đủ `url, title, date_crawled, content_markdown`.
  - `test_standardized_output_covers_both_source_types`: 12 file Markdown chuẩn hóa (>200 ký tự).
